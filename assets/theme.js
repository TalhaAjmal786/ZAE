    ...
    var halo = {
        ...
        headerSearchEvent: function(){
            if ($win.width() < 1025) {
                ...
                if($('[class^="header header-"]').length == 1) {
                    if(!$body.hasClass('open-drawer')){
                        $('.header__search input[name="q"]').on('focus', (event) => {
                            $body.addClass('open-search');
                            $('details.header__seach--details').attr('open','true');
                        });
                    }
        ...
        navSidebarMobile: function() {
            if($('.section-header-navigation').length == 1) {
                ...
                const menuMobileElement = document.getElementById('navigation-mobile');
                ...
                buttonIconOpen.off('click.toggleCurrencyLanguage').on('click.toggleCurrencyLanguage', (event) =>{
                    event.preventDefault();
                    menuSidebarMobileOpen();
                    menuMobileElement.removeAttribute('inert');
                    menuMobileElement.setAttribute('aria-hidden', 'false');
                });

                buttonClose.off('click.toggleCloseCurrencyLanguage').on('click.toggleCloseCurrencyLanguage', () =>{
                    $body.removeClass('menu-open');
                    menuMobileElement.setAttribute('inert', '');
                    menuMobileElement.setAttribute('aria-hidden', 'true');
        ...
        initDropdownCart: function () {
            ...
            const openCart = (trigger) => {
                ...
                const popup = document.querySelector('.halo-quick-cart-popup');
                if (!popup) return;
            ...
            $doc.on('click', '[data-open-cart-popup]', (e) => {
                e.preventDefault();
                e.stopPropagation();
                openCart(e.currentTarget);
            });

            $doc.on('keydown', '[data-open-cart-popup]', (e) => {
            ...
            $doc.on('click', '[data-close-cart-popup]', (e) => {
                e.preventDefault();
                e.stopPropagation();
                closeCart();
            });
            ...
            /* ESC */
            $doc.on('keydown', (e) => {
                if (e.key === 'Escape' && $body.hasClass('cart-dropdown-show')) {
                    closeCart();
                }
            });
            ...
            /* CLICK OUTSIDE */
            $doc.on('click', (e) => {
                if ($body.hasClass('cart-dropdown-show') && !$(e.target).closest('[data-quick-cart-popup]').length && !$(e.target).closest('[data-open-cart-popup]').length) {
                    closeCart();
                }
            });
        },
        ...
        initAskAnExpert: function(){
            this.askAnExpertTrigger = null;
            $doc.on('click', '[data-open-ask-an-expert]', (event) => {
                event.preventDefault();
                event.stopPropagation();
                this.askAnExpertTrigger = event.currentTarget;
                ...
                $.ajax({
                    ...
                    complete: function () {
                        ...
                        requestAnimationFrame(() => {
                            const popup = document.querySelector('[data-ask-an-expert-popup]');
                            const wrapper = popup.querySelector('.halo-modal__dialog');
                            const focusable = wrapper.querySelector(
                                'button:not([disabled]), [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
                            );
                            const closeBtn = wrapper.querySelector('[data-close-ask-an-expert]');
                            wrapper.focus();
                            trapFocus(wrapper, closeBtn);
                        });
                    }
                });
            });
            document.addEventListener('keyup', (event) => {
                if (event.code === 'Escape'){
                    ...
                    removeTrapFocus(this.askAnExpertTrigger);
                }
            });
        ...
        initQuickView: function () {
            let checkLoadQV = true;
            this.quickViewTrigger = null;

            $doc.on('click', '[data-open-quick-view-popup]', (event) => {
                event.preventDefault();
                event.stopPropagation();

                this.quickViewTrigger = event.currentTarget;

                if (checkLoadQV) {
                    checkLoadQV = false;
                    const popup = document.querySelector('.halo-quick-view-popup');
                    halo.buildStyleSheet(popup.dataset.urlStyleProduct, popup);
                    halo.buildStyleSheet(popup.dataset.urlStyleQuickView, popup);
                }

                const handle = $(event.currentTarget).data('product-handle');
            ...
            document.addEventListener('keyup', (event) => {
                if (event.code === 'Escape'){
                    ...
                    removeTrapFocus(this.quickViewTrigger);
                }
            });
            ...
            $doc.on('click', '.background-overlay', (event) => {
                if ($body.hasClass('quick-view-show')) {
                    if (
                        $(event.target).closest('[data-quick-view-popup]').length === 0 &&
                        $(event.target).closest('[data-open-quick-view-popup]').length === 0
                    ) {
                        $body.removeClass('quick-view-show');
                        removeTrapFocus(this.quickViewTrigger);
                    }
        ...
        updateContentQuickView: function (handle) {
            const popup = document.querySelector('[data-quick-view-popup]');
            const wrapper = popup.querySelector('.halo-popup-wrapper');
            ...
            $.ajax({
                ...
                success: (data) => {
                    const html = new DOMParser().parseFromString(data, 'text/html');
                    const section = html.getElementById('shopify-section-halo-quick-view');
                ...
                complete: () => {
                    const $scope = $(popup).find('.quickView');
                    ...
                    if (window.quick_view_buy_it_now && window.Shopify?.PaymentButton) {
                        Shopify.PaymentButton.init();
                    }
                    ...
                    requestAnimationFrame(() => {
                        requestAnimationFrame(() => {
                            ...
                            const closeBtn = wrapper.querySelector('[data-close-quick-view-popup]');
                            wrapper.focus();
                            trapFocus(wrapper, closeBtn);
                        });
                    });
        ...
        clickPlayVideo: function() {
            const playIcon = $('[data-play-video]');
            if (playIcon.length) {
                playIcon.on('click', function(e) {
                    let $video;
                    if ($(window).width() > 1024) {
                        $video = $(e.currentTarget).siblings('video.hidden-mobile');
                    } else {
                        $video = $(e.currentTarget).siblings('video.hidden-desktop');
                    }
